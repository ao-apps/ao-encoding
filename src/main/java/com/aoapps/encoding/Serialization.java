        ...
        for (String value : Strings.split(acceptHeaderValues.next(), ',')) {
          ...
          if (paramsSize > 0) {
            ...
            if (acceptType.equals("*/*")) {
              ...
              if (!hasNegativeQ) {
                if (acceptType.equalsIgnoreCase(ContentType.XHTML)) {
                  hasAcceptApplicationXhtmlXml = true;
                } else if (acceptType.equalsIgnoreCase(ContentType.HTML)) {
                  hasAcceptTextHtml = true;
                } else {
                  throw new AssertionError("Unexpected value for acceptType: " + acceptType);
                }
              }
            }
          }
